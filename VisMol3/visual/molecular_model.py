...
class Frame:
    ...
    def load_bonds(self):
        """
        """
        import visual.operations
        self.bonds = visual.operations.generate_bonds(self.atoms)
    ...
    def load_ribbons(self):
        ...
        for chain in self.chains.values():
            ribs = visual.operations.generate_ribbons(chain.backbone)
            self.ribbons += ribs
...
class Atom:
    ...
    def __init__ (self, name='Xx', index=None, symbol='X', pos=None, residue=None):
        """ Class initialiser """
        import visual.atom_types as at
        if pos is None:
